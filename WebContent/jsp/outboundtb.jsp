...
$(function(){
	...
	var j = new Array();
	var  k=0;
	<c:forEach items="${resQueryouttbList}" var="p">  
	 j[k]=${p.outNumber};
     k=k+1;
	</c:forEach> 
	var name= document.getElementById("out_name").value;
	...
	var arrayObj=name.split(",");
		...
		m[i]=arrayObj[i];
         ...
         r.text(290, 40, "出库饼状分析图").attr({ font: "20px sans-serif" });
         pie.hover(function () {
             this.sector.stop();
...
<!-- 导出按键组件-->
<!-- <input type="button" value="图表模式" class="btn" onclick="changepicture()"/> -->
<div class="container ">
    <div class="row">
...
<input id="out_name" value="${resoutProductName}" type="hidden">
<!-- 图表模式 -->

<div id="holder">
<!-- <input type="button" class="btn" value="表格模式"/ onclick="changetable()"> -->
</div>
